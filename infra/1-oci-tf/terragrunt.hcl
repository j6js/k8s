...
locals {
  regions              = yamldecode(file("${get_terragrunt_dir()}/../shared/oci-config/regions.yaml"))
...
generate "locals" {
  path      = "locals.dynamic.tf"
  if_exists = "overwrite_terragrunt"
  contents  = templatefile("${get_terragrunt_dir()}/locals.dynamic.tf.tftpl", { regions = local.regions, regions_list = local.regions_list, terragrunt_dir = get_terragrunt_dir() })
}
